...
SUMMARY_PATH = EVAL_DIR / "isolation_forest_v2_summary.csv"
MONTHLY_PATH = EVAL_DIR / "monthly_method_comparison_v2.csv"
LINE_PATH = EVAL_DIR / "line_method_comparison_v2.csv"
...
iforest_count = summary["iforest_v2_anomaly_count"]
both_count = summary["both_methods_anomaly_count"]
only_zscore = summary["only_zscore_anomaly_count"]
only_iforest = summary["only_iforest_v2_anomaly_count"]
zscore_rate = summary["zscore_anomaly_rate"]
iforest_rate = summary["iforest_v2_anomaly_rate"]
...
with tab_overview:
    ...
    col3.metric("Isolation Forest V2 Anomalies", format_int(iforest_count), format_percent(iforest_rate))
    ...
    st.markdown(
        """
        **Raw Data → Spark Preprocessing → Hourly Aggregation → Feature Engineering → Z-score → Isolation Forest V2 → Evaluation**
        """
    )
    ...
    pipeline_df = pd.DataFrame(
        {
            ...
            "Description": [
                ...
                "Isolation Forest V2 with refined features",
...
# -----------------------------
with tab_comparison:
    st.header("Z-score vs Isolation Forest V2")

    comparison_df = pd.DataFrame(
        {
            "Method": ["Z-score", "Isolation Forest V2"],
    ...
    with col2:
        ...
        st.markdown(
            ...
            **different anomaly perspectives**.
            ...
            - **Isolation Forest V2** captures multivariate irregular patterns.
            """
        )
...
with tab_monthly:
    ...
    monthly_long = monthly_plot_df.melt(
        id_vars=["month"],
        value_vars=["zscore_anomaly_count", "iforest_v2_anomaly_count"],
        var_name="Method",
    ...
    monthly_long["Method"] = monthly_long["Method"].replace(
        {
            ...
            "iforest_v2_anomaly_count": "Isolation Forest V2",
        }
    )
    ...
    fig = px.line(
        ...
        color="Method",
        markers=True,
        title="Monthly Anomaly Count: Z-score vs Isolation Forest V2",
    )
    ...
    monthly_rate_long = monthly_plot_df.melt(
        id_vars=["month"],
        value_vars=["zscore_anomaly_rate", "iforest_v2_anomaly_rate"],
        var_name="Method",
    ...
    monthly_rate_long["Method"] = monthly_rate_long["Method"].replace(
        {
            ...
            "iforest_v2_anomaly_rate": "Isolation Forest V2",
        }
    )
    ...
    display_monthly["iforest_v2_anomaly_rate"] = display_monthly["iforest_v2_anomaly_rate"].map(lambda x: f"{x * 100:.2f}%")
...
with tab_lines:
    ...
    if filtered_lines.empty:
        ...
        fig = px.bar(
            top_lines.sort_values("iforest_v2_anomaly_rate"),
            x="iforest_v2_anomaly_rate",
            y="line",
            orientation="h",
            title="Top Lines by Isolation Forest V2 Anomaly Rate",
            text="iforest_v2_anomaly_rate",
        )
        fig.update_traces(texttemplate="%{text:.2%}", textposition="outside")
        fig.update_layout(
            xaxis_title="Isolation Forest V2 Anomaly Rate",
        ...
        fig.update_layout(
            ...
            yaxis_title="Isolation Forest V2 Anomaly Rate",
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        display_lines["iforest_v2_anomaly_rate"] = display_lines["iforest_v2_anomaly_rate"].map(lambda x: f"{x * 100:.2f}%")
        ...
        display_lines["iforest_v2_anomaly_count"] = display_lines["iforest_v2_anomaly_count"].map(lambda x: f"{int(x):,}")
...
with tab_interpretation:
    ...
    st.subheader("2. What Isolation Forest V2 adds")
    st.markdown(
        """
        Isolation Forest V2 provides a multivariate perspective. Instead of only
    ...
    st.markdown(
        ...
        - Isolation Forest V2 detects unusual multivariate combinations.
        """
    )
    ...
    st.success(
        ...
        Isolation Forest V2 complements it by revealing additional multivariate
